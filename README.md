...
- in other cases, the reference token is written first, followed by the separator ```||||``` and the corresponding predicted (sub)token(s); tokenisation mismatches between the reference and prediction are marked on the predicted side as follows:
  - when there is a token boundary on the predicted side that does not correspond to a reference token boundary, it is marked using one or two consecutive symbols ```▁```, depending on whether the predicted tokens are white-space separated or not (e.g. (1) ```surtout||||sur▁▁tout```, where the two-token predicted sequence ```sur tout``` is aligned with the reference token ```surtout```; and (2) ```davantage||||d'▁avantage```, where the two-token predicted sequence ```d'avantage```, which is tokenised as ```d' avantage```, is aligned with the reference token ```davantage```);
...
This token-level alignment is produced based on a character-level alignment obtained using an dedicated variant of the weighted Levenshtein algorithm, aimed at avoiding tokenisation mismatches unless they are really necessary for a successful alignment:
